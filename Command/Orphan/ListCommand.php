...
namespace Darvin\FileBundle\Command\Orphan;
...
class ListCommand extends Command
{
    ...
    /**
     * @param string[]                                        $pathnames Orphan file pathnames
     * @param \Symfony\Component\Console\Style\StyleInterface $io        Input/output
     * @param string                                          $title     Title
     *
     * @return ListCommand
     */
    private function print(array $pathnames, StyleInterface $io, string $title): ListCommand
    {
    ...
    private function findInDatabase(string $class): array
    {
        ...
        while ($row = $iterator->next()) {
            ...
            if ($iterator->key() > 0 && 0 === $iterator->key() % $this->chunk) {
                $this->em->clear();
            }
        }
    ...
    private function findInFilesystem(string $class): array
    {
        ...
        foreach ($this->uploaderMetaReader->getUploadableFields($class) as $field => $params) {
            ...
            if (in_array(AbstractFile::class, class_parents($class))) {
